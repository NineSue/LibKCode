...
现代操作系统内核（如 Linux）经过数十年演进，已实现大量高度优化、经过严格验证的数据结构与算法，例如红黑树（`rbtree`）、Maple Tree、特化的堆排序（`sort`）等。这些实现广泛应用于内核调度器、内存管理、文件系统等关键路径，具备极高的可靠性与性能。
...
- **零上下文切换**：稳态调用延迟 < 10ns，比传统 syscall 快 50 倍以上。
- **高性能红黑树**：在 100 万规模数据集上，**查找快 42.5%**，**删除快 18.8%**，全面超越 C++ STL 的 `std::map`。
- **确定性排序**：提供内核原生的堆排序 (`kcode_sort`)，保证最坏情况 $O(n \log n)$ 时间和 $O(1)$ 空间复杂度。
